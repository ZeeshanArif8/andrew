...
class MultiChannelSale(models.Model):
    ...
    def connect_woocommerce(self):
        message = ""
        req = self.get_woocommerce_connection()
        try:
...
#---------------------------------------Update Process -------------------------------------------
    ...
    def update_woocommerce_categories(self, woocommerce, record, initial_record_id, remote_id):
        obj = self.env['export.categories'].create(
            {
                'channel_id': self.id,
                'operation': 'export',
...
#----------------------------------- Core Methods -----------------------------------------------
    ...
    def woocommerce_post_confirm_paid(self, invoice, mapping_ids, result):
        self.woocommerce_set_order_status(order_id=mapping_ids.store_order_id, status='paid')

    def woocommerce_post_cancel_order(self, sale_order, mapping_ids, result):
        self.woocommerce_set_order_status(order_id=mapping_ids.store_order_id, status='cancelled')

    def woocommerce_set_order_status(self, order_id, status):
        order_status = self.order_state_ids.filtered(
            lambda order_state_id: order_state_id.odoo_order_state == status
        )
        if order_status:
            self.get_woocommerce_connection().put(f'orders{order_id}', {'status': order_status[0].channel_state})
    ...
    def sync_quantity_woocommerce(self, mapping, qty):
        ...
        res = self.get_woocommerce_connection().put(
            url,
