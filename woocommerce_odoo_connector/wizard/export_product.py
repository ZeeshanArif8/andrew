...
class ExportWoocommerceTemplates(models.TransientModel):
    ...
    def get_woocommerce_attribute(self, woocommerce, channel, attribute_id,attribute_value_ids):
        if attribute_id:
            ...
            exported = self.export_attribute(attribute_id,attribute_value_ids)
            if exported[0]:
    ...
    def export_attribute(self, attribute,attribute_values):
        ...
        for attribute_value in attribute_values:
            ...
            _logger.info("Attribute with Id(%r) and its values %r are exported/updated to Woocommerce." % (attribute.id, vals))
    ...
    def export_attribute_values(self, woocommerce, channel, attribute_value, store_attribute_id):
        ...
        if not attr_value_mapping:
            ...
            return_dict = woocommerce.post(f'products/attributes/{store_attribute_id}/terms', attribute_value_dict).json()
            if 'message' in return_dict:
    ...
    def create_woocommerce_simple_product(self, woocommerce, channel, template):
        ...
        product_dict = {
            ...
            'regular_price'		: str(template.with_context(pricelist=channel.pricelist_name.id).list_price) or "",
            'type'				: 'simple',
            'categories'		: self.set_woocommerce_product_categories(woocommerce, channel, template),
            'status'			: 'publish',
            'short_description'	: template.description_sale or "",
            'description'		: template.description or "",
            'attributes'		: self.set_woocommerce_attribute_line(woocommerce, channel, template),
            'price'				: template.with_context(pricelist=channel.pricelist_name.id).list_price,
            'manage_stock'		: True,
            'stock_quantity'	 : quantity,
            'in_stock'		       	: True,
        }
